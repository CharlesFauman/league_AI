...
if __name__ == '__main__':
	...
	for match_id, match in matches.items():
		...
		all_data.append(np.column_stack((win_team,lose_team)))
	...
	if not os.path.exists('data/'):
		os.makedirs('data/')
	with open('data/train_data.bin', 'wb') as file:
		np.save(file, train_data)
	with open('data/dev_data.bin', 'wb') as file:
		np.save(file, dev_data)
	with open('data/test_data.bin', 'wb') as file:
